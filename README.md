...
### 为什么比 Tokio 还要快?
难道老王是天才吗?很遗憾并不是.
之所以性能超越 Tokio 是因为 Rain 并没有使用任务窃取,所以节约了跨cpu任务传递的消耗.
那么 Rain 没有应用层自动负载均衡的策略吗?是的, Rain 并没有设计应用层自动负载均衡,而是使用了内核层负载均衡.
对于网络任务,使用`SO_REUSEPORT`,可以让内核自动分配处理连接的cpu核心,达到负载均衡的效果.
因此节约了自动跨核传递任务的消耗.
那 Rain 没有跨核任务传导机制吗?也不是,我们可以使用 `EventLoop::submit` 把任务投递到指定核心,达到跨核传递任务的效果.
